USE [dbRapidDevs]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER PROCEDURE [dbo].[usp_PurchaseBill_Create]
    @supplier_id INT,
    @tax_percentage DECIMAL(5, 2) = NULL,
    @remarks NVARCHAR(1000) = NULL,
    @items dbo.udt_purchase_bill_item READONLY
AS
BEGIN
    SET NOCOUNT ON;
    BEGIN TRANSACTION;

    BEGIN TRY
        IF EXISTS (
            SELECT 1
            FROM @items i
            WHERE NOT EXISTS (
                SELECT 1
                FROM dbo.supplier_product sp
                WHERE sp.product_id = i.ProductId
                  AND sp.supplier_id = @supplier_id
                  AND sp.is_deleted = 0
            )
        )
            THROW 50001, 'One or more products are not supplied by the selected supplier.', 1;

        IF EXISTS (
            SELECT 1
            FROM @items i
            INNER JOIN dbo.purchase_order po
                ON po.purchase_order_id = i.PurchaseOrderId
            WHERE i.PurchaseOrderId IS NOT NULL
              AND po.supplier_id <> @supplier_id
        )
            THROW 50002, 'A linked purchase order belongs to a different supplier.', 1;

        DECLARE @date_part NVARCHAR(8) = CONVERT(NVARCHAR(8), GETDATE(), 112);
        DECLARE @seq_prefix NVARCHAR(20) = 'BILL-' + @date_part + '-';
        DECLARE @next_seq INT;

        SELECT @next_seq = ISNULL(MAX(
            CAST(SUBSTRING(bill_number, LEN(@seq_prefix) + 1, 10) AS INT)
        ), 0) + 1
        FROM dbo.purchase_bill
        WHERE bill_number LIKE @seq_prefix + '%';

        DECLARE @bill_number NVARCHAR(50) =
            @seq_prefix + RIGHT('0000' + CAST(@next_seq AS NVARCHAR), 4);
        DECLARE @subtotal DECIMAL(18, 4);

        SELECT @subtotal = SUM(i.Quantity * ISNULL(i.UnitPrice, sp.purchase_price))
        FROM @items i
        INNER JOIN dbo.product p ON p.product_id = i.ProductId
        LEFT JOIN dbo.supplier_product sp 
            ON sp.product_id = i.ProductId 
           AND sp.supplier_id = @supplier_id 
           AND sp.is_deleted = 0;

        DECLARE @total_amount DECIMAL(18, 4) =
            @subtotal + ISNULL(@subtotal * @tax_percentage / 100, 0);
        DECLARE @new_id INT;

        INSERT INTO dbo.purchase_bill
            (bill_number, supplier_id, tax_percentage, remarks, total_amount, created_at, is_deleted)
        VALUES
            (@bill_number, @supplier_id, @tax_percentage, @remarks, @total_amount, GETDATE(), 0);

        SET @new_id = SCOPE_IDENTITY();

        INSERT INTO dbo.purchase_bill_item
            (purchase_bill_id, product_id, purchase_order_id, purchase_order_item_id,
             quantity, unit_price, created_at, is_deleted)
        SELECT
            @new_id,
            i.ProductId,
            i.PurchaseOrderId,
            i.PurchaseOrderItemId,
            i.Quantity,
            ISNULL(i.UnitPrice, sp.purchase_price),
            GETDATE(),
            0
        FROM @items i
        INNER JOIN dbo.product p ON p.product_id = i.ProductId
        LEFT JOIN dbo.supplier_product sp 
            ON sp.product_id = i.ProductId 
           AND sp.supplier_id = @supplier_id 
           AND sp.is_deleted = 0;

        UPDATE p
        SET p.stock = p.stock + i.Quantity,
            p.modified_at = GETDATE()
        FROM dbo.product p
        INNER JOIN @items i ON i.ProductId = p.product_id;

        INSERT INTO dbo.stock_record
            (product_id, record_type, transaction_id, quantity_change, price, reason, created_at)
        SELECT
            i.ProductId,
            1,
            @new_id,
            i.Quantity,
            ISNULL(i.UnitPrice, sp.purchase_price),
            'Purchase Bill: ' + @bill_number,
            GETDATE()
        FROM @items i
        INNER JOIN dbo.product p ON p.product_id = i.ProductId
        LEFT JOIN dbo.supplier_product sp 
            ON sp.product_id = i.ProductId 
           AND sp.supplier_id = @supplier_id 
           AND sp.is_deleted = 0;

        COMMIT TRANSACTION;
        SELECT @new_id AS new_id;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
        THROW;
    END CATCH;
END
GO